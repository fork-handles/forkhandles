...
class InstanceFabricatorTest {
    ...
    fun `when user expects empty collections, both Map and List are empty`() {
        ...
        repeat(10) {
            assertEquals(
                emptyList<List<Int>>(),
                Fabrikate.random<List<Int>>(config)
            )
            assertEquals(
                emptyList<List<List<Int>>>(),
                Fabrikate.random<List<List<Int>>>(config)
            )
            assertEquals(
                emptyMap<Int, String>(),
                Fabrikate.random<Map<Int, String>>(config)
            )
        }
    }
    ...
    fun `when user expects concrete collection size, both Map and List are of this size`() {
        ...
        repeat(10) {
            assertEquals(5, Fabrikate.random<List<Int>>(config).size)
            assertEquals(5, Fabrikate.random<List<List<Int>>>(config).size)
            assertEquals(5, Fabrikate.random<Map<Int, String>>(config).size)
        }
    }
    ...
    fun `when user expects concrete String length, all Strings have this length`() {
        ...
        repeat(10) {
            assertEquals(5, Fabrikate.random<String>(config).length)
            assertEquals(5, Fabrikate.random<List<String>>(config)[0].length)
            assertEquals(5, Fabrikate.random<List<List<String>>>(config)[1][1].length)
        }
    }
    ...
    fun `object set in config as Any, is always returned when we expect Any`() {
        ...
        repeat(10) {
            assertEquals(any, Fabrikate.random<Any>(config))
            assertEquals(any, Fabrikate.random<GA<Any>>(config).t)
            assertEquals(any, Fabrikate.random<GA<GA<Any>>>(config).t.t)
        }
    }
    ...
    fun `check expected random values`() {
        ...
        assertEquals(
            "A",
            Fabrikate.random<A>(config).toString()
        )

        assertEquals(
            "B(a=A)",
            Fabrikate.random<B>(config).toString()
        )

        assertEquals(
            "C(b1=B(a=A), c=h, b2=B(a=A), str=nGhZZ9O, l=8970572073855783324)",
            Fabrikate.random<C>(config).toString()
        )

        assertEquals(
            "D(a=A)",
            Fabrikate.random<D>(config).toString()
        )
        ...
        assertEquals(
            ...
            Fabrikate.random<E>(config).toString()
        )
        ...
        assertEquals(
            ...
            Fabrikate.random<F>(config).toString()
        )
    }
}
